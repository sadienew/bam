   ...
   // @SuppressWarnings("empty-statement")
    public static void main(String[] args) {
        ...
        for(int i=0;i<players.length;i++)
        {
            ...
            while(ans.toUpperCase().equals("H"))
            {
                ...
                //players[i].calcTotal();
                System.out.printf("Player %d's hand value = %s\n", i+1, players[i].value=players[i].calcTotal());
                System.out.println("");
                
                if(players[i].value > 21)
                {
                        System.out.printf("Player %d, you have busted!\n", i+1);
        ...
        ******************************************************/
        ...
        for(int i=0;i<players.length;i++)
        {
            if(players[i].value == 21)
            {
                players[i].winnings += 1.5 * players[i].bet;
            }
            else if((dealer.value > 21) && (players[i].value < 21))
            {
                players[i].winnings += players[i].bet;
            }
            else if((players[i].value > dealer.value) && (players[i].value < 21))
            {
                players[i].winnings += players[i].bet;
            }
            ...
            System.out.printf("Player %d's winnings = $%d\n",i+1, players[i].winnings);
    ...
    public static void insurance(Player [] players, Dealer dealer, DeckOfCards myDeck)
    {
        ...
        if(dealer.cards[1].points == 10)
        {
            for(int i=0;i<players.length;i++)
            {
                players[i].winnings = players[i].insuranceBet;
            }
        }
    }
        ...
        if(dealer.value == 21)
        {
            ...
            for(int i=0;i<players.length;i++)
            {
                ...
                if(players[i].value == 21)
                {
                    ...
                    players[i].winnings += players[i].bet;
                    System.out.printf("Player %d's winnings: $%s\n",i+1, players[i].winnings);
                }
